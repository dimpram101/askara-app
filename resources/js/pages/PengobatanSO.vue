<template>
   <div class="">
      <!-- Header with Filters -->
      <div class="mx-auto max-w-7xl px-8 py-4 xl:px-4">
      ...
      <!-- Main Content -->
      <div class="mx-auto max-w-7xl px-8 pb-8 xl:px-4">
         <div class="grid grid-cols-1 gap-x-8 gap-y-4 xl:grid-cols-3">
            ...
            >
               <div class="p-6">
                  ...
                  <div class="text-sm">
                     <span class="">+{{ statistics.newPatientsGrowth }}%</span>
                     from Last Year
                  </div>
               </div>
            </div>
...
// Type definitions
type MetricType = 'cases' | 'cured' | 'mortality_rate' | 'incident_rate';
...
interface ProvinceDataItem {
   ...
   deaths?: number;
   success_rate?: number;
   loss_to_follow_up?: number;
   hotspot_average_quadran?: number;
}
...
const selectedMetric = ref<MetricType>('cases');
...
// Map instance - properly typed
let map: L.Map | null = null;
let currentGeoJSONLayer: L.GeoJSON | null = null;
let currentLegend: L.Control | null = null;
let hotspotMarkers: L.CircleMarker[] = [];

// Reactive data from API
const apiData = ref<ApiDataItem[]>(props.apiData || []);
const provinceData = ref<Record<string, ProvinceDataItem>>({});

// Fetch data from API
const fetchTBData = async (): Promise<void> => {
...
const processProvinceData = (): void => {
   try {
      const processed: Record<string, ProvinceDataItem> = {};
      ...
      if (map && !currentGeoJSONLayer) {
         console.log('Reloading Indonesia choropleth layer');
         setTimeout(async () => {
...
const loadFallbackData = (): void => {
   console.warn('Loading fallback data due to API error');
   provinceData.value = {
      ACEH: {
         cases: 10933,
         cured: 1320,
...
const getMetricRanges = (metric: MetricType) => {
   const allValues = Object.values(provinceData.value)
      .map((data: ProvinceDataItem) => data[metric])
...
const getColorByMetric = (provinceName: string, metric: MetricType): string => {
...
const updateLegend = async () => {
   ...
   const colorSchemes: Record<MetricType, string[]> = {
...
const addHotspotMarkers = async (): Promise<void> => {
   try {
      const L = (await import('leaflet')).default;

      // Clear existing hotspot markers
      hotspotMarkers.forEach((marker: L.CircleMarker) => {
      ...
      console.error('Error adding hotspot markers:', error);
   }
};
...
const loadIndonesiaChoropleth = async () => {
   ...
   const getColor = (provinceName: string): string => {
          ...
          `);
            ...
            <div class="p-3 max-w-xs">
              ...
              <div class="text-xs text-gray-600">
                ...
                <strong>Normalized Name:</strong> ${normalizedName || 'null'}<br>
                <strong>Available in API:</strong> ${normalizedName && Object.keys(provinceData.value).includes(normalizedName) ? 'Yes' : 'No'}
              </div>
            </div>
          `);
            }
...
const initMap = async () => {
   try {
      ...
      if (Object.keys(provinceData.value).length > 0) {
         ...
         console.warn('No province data available, choropleth map not loaded');
      }
   } catch (error) {
...
onMounted(async () => {
   isMapLoading.value = true;
   await fetchTBData(); // Fetch data first
   await nextTick();

   // Wait a bit more to ensure data is processed
   setTimeout(async () => {
      await initMap();
      isMapLoading.value = false;
   }, 500);
